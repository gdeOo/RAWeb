...
if ($_FILES["file"]["size"] > 1048576) {
...
} elseif ($extension == null || mb_strlen($extension) < 1) {
...
} elseif (!in_array($uploadType, $allowedTypes)) {
    ...
    if ($uploadType == "NEWS") {
        $maxImageSizeWidth = 160;
        $maxImageSizeHeight = 160;
    } elseif ($uploadType == "GAME_ICON") { //	ICON
        $maxImageSizeWidth = 96;
        $maxImageSizeHeight = 96;
    } elseif ($uploadType == "GAME_TITLE" || $uploadType == "GAME_INGAME") {  //	Screenshot
        $maxImageSizeWidth = 320;
        $maxImageSizeHeight = 240;
    } elseif ($uploadType == "GAME_BOXART") {
        $maxImageSizeWidth = 320;
    ...
    if ($success == false) {
        ...
        if ($uploadType == "NEWS") {
            ...
            exit;
        } elseif ($uploadType == "GAME_ICON" || $uploadType == "GAME_TITLE" || $uploadType == "GAME_INGAME" || $uploadType == "GAME_BOXART") {
            //	Associate new data, then return to game page:

            $param = '';
            if ($uploadType == "GAME_ICON") {
                $param = 'ImageIcon';
            } elseif ($uploadType == "GAME_TITLE") {
                $param = 'ImageTitle';
            } elseif ($uploadType == "GAME_INGAME") {
                $param = 'ImageIngame';
            } elseif ($uploadType == "GAME_BOXART") {
                $param = 'ImageBoxArt';
            }
